...
exports.getClassStudents = async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.classInCharge) {
      return res
        .status(404)
        .json({ message: "User or class in charge not found" });
    }
...
exports.addGrade = async (req, res) => {
  ...
  try {
    ...
    if (studentGrade) {
      ...
      res
        .status(201)
        .json({ message: "Grade added successfully", studentGrade });
    } else {
...
exports.getStudentGrade = async (req, res) => {
  ...
  try {
    ...
    if (!studentGrades.length) {
      return res
        .status(404)
        .json({ message: "Grade not found for the student" });
    }
    ...
    res
      .status(200)
      .json({ message: "Grades retrieved successfully", grades: response });
  } catch (error) {
...
exports.addTask = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error Adding Task", error: error.message || error });
  }
};

exports.assignedTask = async (req, res) => {
  try {
    const task = await Task.find({ teacher: req.user._id }).populate(
      "assignedTo",
      "name"
    );
...
exports.getNotice = async (req, res) => {
  try {
    ...
    // Find only upcoming events (date is in the future)
    const upcomingNotice = await Notice.find({
      date: { $gt: currentDate },
    }).sort({ date: 1 });
